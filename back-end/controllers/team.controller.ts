      ...
      for (const m of members) {
        ...
        if (!teamMember) {
          teamMember = new User({
            name: m.name,
            jobTitle: m.jobTitle,
            // email: ??? (you can make it optional or generate temp)
            // password: not needed if you use magic link / invite later
            role: "Member",           // or "Pending" if you want invite flow
            isActive: false,          // until they accept / register
            teamId: team._id,
          });
...
export const getTeamMembers = async (req: Request, res: Response) => {
  try {
    ...
    console.log("team._id type:       ", typeof team._id);
    console.log("team.members:        ", team.members);
    console.log("team.members types:  ", team.members.map(m => typeof m));
    console.log("team.members length: ", team.members.length);
    console.log("first member example:", team.members[0]);
    console.log("requester user.id:   ", user.id);
    console.log("requester user._id?: ", user._id);
    console.log("─".repeat(60));

    // ────────────────────────────────────────────────
// Diagnostic block — keep until fixed
// ────────────────────────────────────────────────
